...
You can visit http://34.227.160.238/ or http://ec2-34-227-160-238.compute-1.amazonaws.com/ for the website deployed.
...
### Step 1: Start a new Ubuntu Linux server instance on Amazon EC2 
  ...
  - we can download a .pem file and connect with following command
    ```
    ssh -i 20_2_19_itemcatalog.pem ubuntu@34.227.160.238
    ``` 
...
- To check port 2200 wether working or not by `ssh -i 20_2_19_itemcatalog.pem -p 2200 ubuntu@34.227.160.238` 
...
### Step 7: Create an SSH key pair for `grader`
  ...
  - Restart SSH: `sudo service ssh restart`
  - On the local machine, cheking if the grader account working or not by running this command : `ssh -i 20_2_19_itemcatalog.pem -p 2200 ubuntu@34.227.160.238`.
...
- While logged in as `grader`, configure the time zone: `sudo dpkg-reconfigure tzdata`.
  It is shows timezones, selct no of the above then it shows another list.
...
- Click `Credentials`.
- Create an OAuth Client ID (under the Credentials tab), and add http://34.227.160.238.xip.io and 
http://ec2-34-227-160-238.compute-1.amazonaws.com/ as authorized JavaScript 
origins.
...
- Open your browser to http://34.227.160.238 or http://ec2-34-227-160-238.compute-1.amazonaws.com .
...
- GitHub Repositories 
  - [boisalai/linux-server-configuration](https://github.com/boisalai/udacity-linux-server-configuration)
